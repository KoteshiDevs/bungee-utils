...
public class ControlCommand extends Command implements TabExecutor {
    ...
    public ControlCommand(BungeeUtilsPlugin plugin) {
        super("control");
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (!sender.hasPermission("bungeeutils.control")) {
            sender.sendMessage(new TextComponent(StringUtils.color(MessageUtils.message(MessageUtils.ErrorMessage.NO_PERMISSION))));
            return;
        }

        if (!(args.length == 1)) {
            sender.sendMessage(new TextComponent(StringUtils.color(MessageUtils.message(MessageUtils.ErrorMessage.COMMAND_EXAMPLE).replace("[COMMAND]", "/CONTROL PLAYER"))));
            return;
        }
        ...
        if (controlHandler.getProxiedPlayers().contains(target)) {
            sender.sendMessage(new TextComponent(StringUtils.color(MessageUtils.message(MessageUtils.ErrorMessage.CONTROL_ALREADY))));
            return;
        }

        controlHandler.send(player, target);
    }

    @Override
    public Iterable<String> onTabComplete(CommandSender sender, String[] args) {

        if (!(sender instanceof ProxiedPlayer) || args.length != 1 || !sender.hasPermission("bungeeutils.control")) return ImmutableSet.of();
